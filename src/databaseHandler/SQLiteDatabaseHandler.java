...
public abstract class SQLiteDatabaseHandler {
	/*This class is extended by other classes, so that they can use these standard 
	 * functions for their particular instance of the database.
	 * */
	...
	protected SQLiteDatabaseHandler(String databaseFileName){
		...
		try{
			this.conn = connectToDatabase(databaseFileName);
...
//	        System.out.println("Successfully set up connection.");
	        ...
	        System.out.println("\n\tERROR in SQLiteDatabaseHandler.SQLiteDatabaseHandler(): if this says 'out of memory', then the database file is not found.");
	...
	private Connection connectToDatabase(String databaseFileName) throws SQLException{
...
//        }
	...
	private int trySQLModifyQueryExecute(String modifyingQuery){ 
		...
		if(isConnected()){
			try{
				...
				System.err.println(e.getMessage());
				e.printStackTrace();
				System.out.println("\n\tERROR in SQLiteDatabaseHandler.trySQLModifyQueryExecute() : cannot modify database. "
	...
	private ResultSet trySQLSelectQueryExecute(String selectingQuery){ 
		...
		System.out.println("\n\tERROR in SQLiteDatabaseHandler.trySQLSelectQueryExecute() : not connected to database.");
	...
	protected String[] listTablesNames(){				
		...
		if(tablenames==null)
			System.out.println("\n\tERROR in SQLiteDatabaseHandler.listTablesNames() : error in converting ArrayList<String> to String[].");
		return tablenames;
	}
	
	
	
	protected boolean tryCreateTable(String tableDefinition, boolean overwrite){
		String tablename="";
...
//		
		if(tablenames==null){
			System.out.println("\n\tERROR in SQLiteDatabaseHandler.tryCreateTable(): cannot get tablenames in database.");
			return false;
		}
		for(int i=0; i<tablenames.length; i++){
		...
		System.out.println("\n\tERROR in SQLiteDatabaseHandler.tryCreateTable(): no table '"+tablename+"' in database has been created.");
		return false;
//		return true;
	}
	...
	protected String convertDateToString(Date date){	//uses java.sql.Date
		// returns a String of the form: "2016-02-23"
		return date.toLocalDate().toString();	//do NOT import java.util.Date for this.
	}
	
	protected Date convertStringToDate(String date){	//uses java.sql.Date
		date=date.trim();
		String[] dateParts= date.split("-");
		if(dateParts.length!=3){
			System.out.println("\n\tERROR in SQLiteDatabaseHandler.convertStringToDate() : "+date+" is not a valid date-String. \"2016-02-23\" is valid.");
			return null;
		}
	...
	protected String convertTimeToString(Time time){	//uses java.sql.Time
		return time.toString();
	}
	
	protected Time convertStringToTime(String time){	//uses java.sql.Time
		time=time.trim();
		String[] timeParts= time.split(":");
		if(timeParts.length!=3){
			System.out.println("\n\tERROR in SQLiteDatabaseHandler.convertStringToTime() : "+time+" is not a valid time-String. \"05:30:59\" is valid.");
			return null;
		}
	...
	protected ResultSet trySelect(String tablename, String[] fieldsList, String whereClauses, String groupByClauses, String orderByClauses){
		int i;
		if(checkForSQLInjection(tablename) || checkForSQLInjection(whereClauses) || checkForSQLInjection(groupByClauses) || checkForSQLInjection(orderByClauses)){
			System.out.println("\n\tERROR in SQLiteDatabaseHandler.trySelect() : no SQL injection allowed.");
			return null;
		}
	...
	protected ResultSet tryArbitrarySelect(String selectQuery){
		if(checkForSQInjectionModification(selectQuery)){
			System.out.println("\n\tERROR in SQLiteDatabaseHandler.tryArbitrarySelect() : no SQL injection (DELETE, UPDATE, INSERT) allowed.");
			return null;
		}
		return trySQLSelectQueryExecute(selectQuery);
	}
	...
	protected boolean tryInsert(String tablename, String[] fieldsList, String[][] dataList){
		/*
			Example usage:
		...
		for(i=0; i<dataList.length; i++){
			...
			for(j=0; j<dataList[i].length; j++){
				if(dataList[i][j].equals(""))
					...
					try{
						dataList[i][j]=""+Double.parseDouble(dataList[i][j]);	//for numeric values, we must not surround with quotes.
					}catch(NumberFormatException e){
						dataList[i][j]="'"+dataList[i][j]+"'";	//for strings, we must surround with quotes.
					}
				}
	...
	protected boolean tryDelete(String tablename, String whereClauses){
		if(checkForSQLInjectionRetrieval(tablename) || checkForSQLInjectionRetrieval(whereClauses)){
			System.out.println("\n\tERROR in SQLiteDatabaseHandler.tryDelete() : no SQL injection allowed.");
			return false;
		}
